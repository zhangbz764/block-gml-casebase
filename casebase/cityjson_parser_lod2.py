...
# ── 工具函数：统一高度和层数验证逻辑（与 lod1 保持一致）──

def _validate_height(height, surfaces):
    """
    确保 height > 0，否则用 RoofSurface / GroundSurface 的 Z 坐标重算。
    返回有效的 height 值，或 None（无法确定有效高度时丢弃该建筑）。

    surfaces: {"RoofSurface": [(poly, id), ...], "GroundSurface": [...], "WallSurface": [...]}
    """
    if height is not None and height > 0:
        return float(height)

    # height 为 None / <= 0，尝试用 Z 坐标
    roof_faces = surfaces.get("RoofSurface", [])
    ground_faces = surfaces.get("GroundSurface", [])

    if roof_faces and ground_faces:
        roof_z = max(c[2] for poly, _ in roof_faces for c in poly.exterior.coords)
        ground_z = min(c[2] for poly, _ in ground_faces for c in poly.exterior.coords)
        h = roof_z - ground_z
        if h > 0:
            return h
    elif roof_faces and not ground_faces:
        # 只有顶面，用顶面 Z 范围估算（慎用）
        z_values = [c[2] for poly, _ in roof_faces for c in poly.exterior.coords]
        if z_values:
            h = max(z_values) - min(z_values)
            if h > 0:
                return h
...
def _get_ground_roof_from_surfaces(surfaces):
    """从 surfaces 字典中提取 ground_face 和 roof_face，用于 Z 坐标计算高度。"""
    ground_faces = surfaces.get("GroundSurface", [])
    roof_faces = surfaces.get("RoofSurface", [])
    ground_face = ground_faces[0][0] if ground_faces else None
    roof_face = roof_faces[0][0] if roof_faces else None
    return ground_face, roof_face
...
def parse_cityjson_lod2(filepath, target_lod="2"):
    ...
    for obj_id, obj in data["CityObjects"].items():
        ...
        # 统一验证高度
        height = _validate_height(height_raw, surfaces)
        if height is None:
            continue
...
def parse_cityjson_lod2_NL_AM(filepath, target_lod="2.2"):
    ...
    for obj_id, obj in data["CityObjects"].items():
        ...
        # 统一验证高度（≤0 则用 Z 坐标重算）
        height = _validate_height(height, surfaces)
        if height is None:
            continue
...
def parse_cityjson_lod2_CH_ZU(filepath, target_lod="2"):
    ...
    for obj_id, obj in data["CityObjects"].items():
        ...
        height_raw  = attrs.get("measuredHeight") or (
                        (attrs.get("DACH_MAX", 0) - attrs.get("GELAENDEPUNKT", 0))
                        if "DACH_MAX" in attrs and "GELAENDEPUNKT" in attrs
                        else None
                      )
        floor_count_raw = round(height_raw / 3.0) if height_raw else None
        function    = attrs.get("OBJEKTART")
        roof_type   = None
        ...
        # 统一处理层数（覆盖原来的 round(height/3) 逻辑）
        floor_count = _validate_floor_count(floor_count_raw, height)
...
def parse_cityjson_lod2_AT_LZ(filepath, target_lod="3"):
    ...
    for obj_id, obj in data["CityObjects"].items():
        ...
        height_raw  = attrs.get("LoD1 Hoehe")
        floor_count_raw = round(height_raw / 3.0) if height_raw else None
        function    = None
        roof_type   = None
        year_built  = None
    ...
    for obj_id, obj in data["CityObjects"].items():
        ...
        for shell in boundaries:
            for face in shell:
                ...
                if len(coords) >= 3:
                    poly  = Polygon(coords)
                    stype = infer_surface_type(poly, base_z, height_raw)
                    surfaces[stype].append((poly, None))
...
def parse_cityjson_lod2_CZ_PR(filepath):
    ...
    for obj_id, obj in data["CityObjects"].items():
        ...
        attrs       = {**parent_attrs, **own_attrs}

        roof_type   = attrs.get("usage")
        function    = None
        year_built  = None
        ...
        try:
            ...
            if roof_z_values and ground_z_values:
                height_raw = max(roof_z_values) - min(ground_z_values)
        except:
            pass
        ...
        # 统一处理层数
        floor_count = _validate_floor_count(None, height)
...
def parse_gml_lod2(filepath,
    ...
    def parse_building(bldg_el):
        ...
        height_raw  = float(get_attr(col_height)) if col_height and get_attr(col_height) else None
        floor_count_raw = get_attr(col_floor_count) if col_floor_count else None
        floor_count_raw = int(float(floor_count_raw)) if floor_count_raw else None
